...
/* Keep the trip choice separate from the date/search form while preserving the
   existing mode-switch behavior and form submission contract. */
(()=>{
  const form=document.getElementById('searchForm');
  const tripType=document.querySelector('.heroSearchShell .tripTypeField');
  const description=document.getElementById('modeDescription');
  if(!form||!tripType||!description)return;
  form.before(tripType);
  tripType.after(description);
})();
